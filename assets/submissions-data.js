(() => {
  ...
  async function approvedJobs(){const sb=client();if(!sb)return[];const r=await sb.from("public_job_submissions").select("id,title,company,location,url,summary,source_type,created_at").order("created_at",{ascending:false});if(r.error)throw r.error;return r.data||[];}
