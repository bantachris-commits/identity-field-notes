...
function shareTools(a,index=null){
 ...
 <span class="ifn-share-label">${label}</span>
...
async function renderHome(){
 ...
 $("#hero").insertAdjacentHTML("beforeend",shareTools(a));
...
async function renderArticle(){const id=new URLSearchParams(location.search).get("id"),data=await loadJSON("data/articles.json"),a=data.find(x=>x.id===id)||data[0],human=isGuest(a);document.title=`${a.title} — Identity Field Notes`;$("#hero").innerHTML=`<div class="eyebrow">${esc(a.edition)}</div><h1>${esc(a.title)}</h1><p class="lede">${esc(a.dek)}</p>${human?authorLine(a):''}<div class="meta"><span>${fmtDate(a.date)}</span><span>${esc(a.readTime)}</span><span>${human?'Human-written guest contribution':'AI-assisted / source-linked'}</span></div>${tags(a.tags)}<div class="transparency-box"><span class="stamp">${human?'HUMAN-WRITTEN':'DISCLOSURE'}</span><p>${esc(a.disclosure||(human?"Human-written guest contribution.":"AI-generated summary. Verify important details at the original source."))}</p></div>${shareTools(a)}`;$("#stories").innerHTML=a.stories.map((s,i)=>story(s,human,a,i)).join("");scrollToSharedStory();setupComments(a.id);setupNewsletter()}
