(()=>{
  ...
  const RETIRED_LISTING_ID='2026-09-11-machine-identities';
  let notes=[];
  ...
  function retireWeakListing(root=document){
    root.querySelector(`a[href*="${RETIRED_LISTING_ID}"]`)?.closest('.archive-item,.card')?.remove();
  }
  ...
  function installArchive(){
    ...
    list.before(section);
    const observer=new MutationObserver(()=>retireWeakListing(list));
    ...
    setTimeout(()=>retireWeakListing(list),150);
    filterArchive();
  }
  ...
  function installHome(){
    ...
    const weak=host.querySelector(`a[href*="${RETIRED_LISTING_ID}"]`)?.closest('.card');
